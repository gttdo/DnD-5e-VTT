  ...
  const castSpell = async (name: string, level: number, baseLevel = level) => {
    ...
    if (mech?.lingering && mech.area) {
      ...
      onAttack({
        ...
        attackBonus: 0,
        burst: true,
        placeArea: { shape: mech.area.shape, size: mech.area.size, damageType: mech.damageType, level, movable: mech.movable },
        save: mech.save,
  ...
  const castMonsterSpell = async (spellName: string, level: number) => {
    ...
    if (mech?.lingering && mech.area) {
      onAttack({
        ...
        attackBonus: 0,
        burst: true,
        placeArea: { shape: mech.area.shape, size: mech.area.size, damageType: mech.damageType, level: lvl, movable: mech.movable },
        save: mech.save,
        dc: dc ?? undefined,
        condition: mech.condition,
        range,
